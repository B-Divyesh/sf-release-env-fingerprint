...
function routeMeta(route: Route): [string, string, string] {
  if (route === 'demo') return ['Demo — Release Env Fingerprint', 'Compare five release setting differences in an isolated browser sample.', window.location.pathname === '/demo' ? '/demo' : '/?demo=1'];
  ...
  return ['Release Env Fingerprint — compare release settings', 'Compare release settings without exposing secrets.', '/'];
}
...
function differences(baseline: Map<string, Entry>, candidate: Map<string, Entry>): Difference[] {
  ...
  baseline.forEach((entry, name) => {
    ...
    else if (approvedNonSecrets.has(name) && entry.value !== undefined && next.value !== undefined && entry.value !== next.value) rows.push({ state: 'resolved', symbol: '≈', name, detail: 'Value marked non-secret differs' });
  });
  ...
  requestAnimationFrame(() => {
    try {
      ...
      if (rows.length) {
        ...
        output.append(list);
      } else output.append(make('p', 'match-copy', 'The variable names, types, and values marked non-secret match.'));
    } catch (error) {
